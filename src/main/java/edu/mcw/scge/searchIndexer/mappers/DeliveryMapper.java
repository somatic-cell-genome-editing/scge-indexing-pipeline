...
public class DeliveryMapper implements Mapper {
    ...
    public void mapFields(List<ExperimentRecord> experimentRecords, IndexDocument indexDocument) throws Exception {
        ...
        Set<Long> deliverySystemIds = experimentRecords.stream().map(e -> e.getDeliverySystemId()).collect(Collectors.toSet());
        for (long id : deliverySystemIds) {
            if (id != 0) {

                Delivery delivery = deliveryDao.getDeliverySystemsById(id).get(0);
